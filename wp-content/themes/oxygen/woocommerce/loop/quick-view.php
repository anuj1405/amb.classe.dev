					...
					<div class="row spread-2">
						...
						<div class="col col-md-6">
							...
							<div class="product-gallery-env">
								...
								<?php if( get_data( 'shop_sale_ribbon_show' ) &&  $product->is_in_stock() == false && ! ($product->is_type('variable') && $product->get_total_stock() > 0)) : ?>
									<div class="ribbon out-of-stock">
										<div class="ribbon-content">
											<?php echo apply_filters( 'woocommerce_sale_flash', '<span class="outofstock">' . __( 'Out of stock', 'oxygen' ) . '</span>', $post, $product ); ?>
										</div>
									</div>
								<?php elseif( get_data( 'shop_sale_ribbon_show' ) &&  $product->is_on_sale() ) : ?>
									<div class="ribbon">
										<div class="ribbon-content">
											<?php echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale">' . __( 'Sale', 'oxygen' ) . '</span>', $post, $product ); ?>
										</div>
									</div>
								<?php elseif( get_data('shop_featured_product_ribbon_show') && $product->is_featured()): ?>
