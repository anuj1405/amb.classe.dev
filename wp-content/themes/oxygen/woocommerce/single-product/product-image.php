...
<div class="main-images product-images">
	...
	<?php
		if ( has_post_thumbnail() || count($attachment_ids) ) {
			...
			if ( strlen( $image_title ) > 6 && ! preg_match( "/\s+/", $image_title ) ) {
				$image_title = '';
			}
			...
			echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" title="' . $image_title . '" class="woocommerce-main-image zoom" data-lightbox-gallery="main-images">%s %s</a>', $image_link, $image, $zoom ), $post->ID );
			...
			if ( $attachment_ids )
			{
				...
				foreach ( $attachment_ids as $attachment_id )
				{
					...
					echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" title="' . $image_title . '" class="woocommerce-main-image zoom hidden" data-lightbox-gallery="main-images">%s %s</a>', $image_link, $image, $zoom ), $post->ID );
				}
			}
